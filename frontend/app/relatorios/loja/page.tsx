...
function LojaReportContent() {
  ...
  const getGrossAmount = (s: any) => {
    return s.fiscalSource === 'fiscal_document' ? (s.fiscalTotalAmount ?? s.nfeValue ?? 0) : (s.totalParticularAmount ?? 0);
  };
  ...
  const getFiscalQuote = (s: any) => {
    if (s.fiscalSource === 'fiscal_document') {
      return s.fiscalWeightKg !== undefined && s.fiscalUnitPrice !== undefined
        ? s.fiscalWeightKg * s.fiscalUnitPrice
        : undefined;
    }
  ...
  const totalKg = visibleData.reduce((acc, s) => acc + (s.fiscalSource === 'fiscal_document' ? (s.fiscalWeightKg || 0) : (s.totalKg || 0)), 0);
  ...
  function formatFiscal(value: number | undefined, decimalPlaces?: number) {
    if (value === undefined || value === null) return '-';
    return value.toLocaleString('pt-BR', { minimumFractionDigits: decimalPlaces ?? 2, maximumFractionDigits: decimalPlaces ?? 2 });
  }
  ...
  const getProductCellData = (sale: any, productName: string) => {
    ...
    const totalKg = sale.fiscalSource === 'fiscal_document'
      ? sale.fiscalWeightKg
        ...
        <table className="loja-table" style={{ minWidth: uniqueProducts.length > 0 ? `${1000 + uniqueProducts.length * 150}px` : '1000px' }}>
          ...
          <tbody>
            {groupedData.map((group, groupIndex) => {
              ...
              return (
                <Fragment key={group.key}>
                  ...
                  <tr className="group-header-row" style={{ background: groupHeaderBgColor, borderBottom: '2px solid #cbd5e1', pageBreakBefore: groupIndex > 0 ? 'always' : 'auto' }}>
                    <td 
                      colSpan={getColSpan()} 
                  ...
                  <tr className="subtotal-row" style={{ background: subtotalBgColor, fontWeight: 'bold', borderBottom: '2px solid #cbd5e1' }}>
                    ...
                    <td>{money(group.items.reduce((sum, s) => sum + (getFiscalQuote(s) || 0), 0))}</td>
            ...
            <tr className="totals-row">
              ...
              <td>{money(visibleData.reduce((sum, s) => sum + (getFiscalQuote(s) || 0), 0))}</td>
